...
//Rare exceptions to indicate a bug in the _core code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_core = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_core]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.core.module"
